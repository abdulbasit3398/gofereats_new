						...
						<div class="order-item text-center text-md-left d-md-flex">
							...
							<div class="order-info d-flex flex-column justify-content-between">
								...
								<div class="order-method">
									...
									@if($order_row->tips > 0)
										<span>
											{{trans('messages.profile_orders.tips')}} : <span class="currency1"> {!!$order_row->currency->code!!} </span> {{$order_row->tips}}
										</span>
										@endif
									<span>
									...
									<span>
										{{trans('messages.modifiers.delivery_type')}}  : @lang('messages.modifiers.'.$order_row->delivery_type)
									</span>
									<span>
							...
							<div class="order-status d-flex flex-column text-md-right">
								...
								@if($order_row->store->status !=0 )
								<a class="btn btn-theme mt-2" href="newdetails/{{$order_row->store->id}}">{{trans('messages.profile_orders.view_store')}}</a>
								@endif
							</div>
						</div>
						@endforeach
				...
				<div class="tab-pane fade" id="upcoming" role="tabpanel" aria-labelledby="upcoming-tab">
					...
					<div class="order-list">
						...
						<div class="order-item text-center text-md-left d-md-flex">
							...
							<div class="order-info d-flex flex-column justify-content-between">
								<a href="{{route('order_track',['order_id'=>$upcoming_order_row->id])}}">
									...
									<div class="order-method">
										...
										@endif
										@if($upcoming_order_row->tips>0)
										<span>
											{{trans('messages.profile_orders.tips')}} : <span class="currency1"> {!!$upcoming_order_row->currency->code!!} </span> {{$upcoming_order_row->tips}}
										</span>
										@endif
										<span>
										...
										<span>
											{{trans('messages.modifiers.delivery_type')}}   : @lang('messages.modifiers.'.$upcoming_order_row->delivery_type)
										</span>
										<span>
							...
							<div class="order-status d-flex flex-column text-md-right">
								@if($upcoming_order_row->store->status !=0)
								<a class="btn btn-theme mt-auto" href="newdetails/{{$upcoming_order_row->store->id}}">{{trans('messages.profile_orders.view_store')}}</a>
								@endif
					...
					<div class="checkout-list">
						<div class="checkout-item d-flex px-0" ng-repeat="receipt_row in order_detail.order_item">
							...
							<div class="col-6 col-md-7 pl-0">
								<h4>
									<span class="resp_name">@{{receipt_row.menu_name}}</span>
									<span ng-bind-html="currency_symbol"></span>@{{receipt_row.price}}
									<div class="modifier-info" ng-repeat="order_modifier in receipt_row.order_item_modifier">
										<div ng-repeat="order_modifier_item in order_modifier.order_item_modifier_item" >
											<span class="text-truncate"> @{{ order_modifier_item.count | number }} x @{{ order_modifier_item.name }} </span>
											<span class="d-inline-block text-nowrap ml-1"> ( <span ng-bind-html="currency_symbol"></span> @{{ order_modifier_item.count * order_modifier_item.price | number : '2' }} ) </span>
										</div>
									</div>
								</h4>
							</div>
					...
					<div class="total">
						<div class="checkout-total px-0">
							<div class="row d-block">
								...
								<div class="d-flex align-items-center" ng-if="order_detail.tips>0">
									<div class="col-7">
										<p>{{trans('messages.profile_orders.tips')}}</p>
									</div>
									<div class="col-5 text-right">
										<p><span ng-bind-html="currency_symbol"></span>@{{order_detail.tips}}</p>
									</div>
								</div>
								<div class="col-12">
