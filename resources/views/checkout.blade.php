...
@section('main')
<main id="site-content" role="main" ng-init="order_data = {{json_encode($order_detail_data)}};other_store='no';schedule_time_value={{json_encode(time_data('schedule_time'))}};schedule_date_value='{{session('schedule_data')['date']?:date('Y-m-d')}}';schedule_time_set='{{session('schedule_data')['time']}}'; paypal='{{$paypal}}'; stripe='{{$stripe}}'; cash='{{$cash}}'; payment_method={{$default_payment}};paypal_error='{{isset($paypal_error) ? $paypal_error : ''}}'" ng-controller="stores_detail"class="place_order_change">
	<div class="checkout-content py-4 pt-md-5" ng-init="date='{{date('d M',strtotime($schedule_data['date']))}}'" ng-class="ajax_loading ? 'loading':''">
		<div class="container" ng-init="time='{{date('h:i A',strtotime($schedule_data['time']))}}'">
			<div class="clearfix">
				<div class="checkout-detail col-12 col-md-7 col-lg-8 pl-0 pr-0 pr-md-4 float-left">
					<h1>{{ trans('messages.store.checkout') }}</h1>
					<div class="panel-group">
						...
						<div class="card done user_check1">
							...
							<div class="card-body">
								...
								<div class="card-btn mt-4">
									<a href="{{route('login')}}" class="btn btn-primary mr-2">{{ trans('messages.profile.sign_in') }}</a>
						...
						<div class="card active user_check2">
							...
							<div class="card-body schedule3" ng-init="status='{{$schedule_data['status']}}'">
								<p>
									...
									<a href="#" class="float-right schedule-btn theme-color" data-toggle="modal" data-target="#schedule-modal" ng-if="status=='ASAP'">{{ trans('messages.store.schedule') }}</a>
									
									<a href="#" class="float-right schedule-btn theme-color" id="scheduleasap" ng-if="status!='ASAP'">{{ trans('admin_messages.as_soon_as_possible') }}</a>
						...
						<div class="card active user_check2">
							<div class="card-header">
								<h3>3. {{ trans('messages.profile_orders.delivery_type') }}</h3>
							</div>
							<div class="card-body delivery_type">
								<div class="form-group">
									@foreach($store_delivey_type as $delivery_type)
									 <label>
					                    <input type="radio" class="require" name="delivery_type" value={{$delivery_type}} >
					                    @lang('messages.modifiers.'.$delivery_type)
					                  </label>
					                  @endforeach
								</div>
							</div>
								...
								<div class="check-location w-100 pl-0 pl-md-4 mt-4 mt-md-0">
									<div class="form-group">
										<div class="search-input">
											...
											<input type="text" class="w-100 text-truncate" placeholder="{{ trans('messages.store.enter_your_address') }}" id="confirm_address" value="{{session('location')}}" />

											<input type="hidden" id="store_id" value="{{$store_details->id}}">

											<input type="hidden" name="order_city" id="order_city" value="{{$user_details->user_address->city}}">
											<input type="hidden" name="order_street" id="order_street" value="{{$user_details->user_address->city}}">
											<input type="hidden" name="order_state" id="order_state" value="{{$user_details->user_address->state}}">
											<input type="hidden" name="order_country" id="order_country" value="{{$user_details->user_address->country}}">
						...
						<div class="card active user_check2 promo_loading">
							<div class="card-header">
								<h3>5. @lang('messages.profile.payment') </h3>
							</div>
							<div class="card-body" ng-inoi>
								<!-- <div class="custom-control custom-radio custom-control-inline">
								  <input type="radio" id="customRadioInline1" name="customRadioInline1" class="custom-control-input">
								  <label class="custom-control-label" for="customRadioInline1">{{ trans('messages.store.cash_paid_at_delivery') }}</label>
								</div>
								<div class="custom-control custom-radio custom-control-inline">
								  <input type="radio" id="customRadioInline2" name="customRadioInline1" class="custom-control-input">
								  <label class="custom-control-label" for="customRadioInline2">{{ trans('messages.store.debit_or_credit_card') }}</label>
								</div>
								<div class="custom-control custom-radio custom-control-inline">
								...
								<select id="payment_method" ng-model="payment_method" ng-change="updatePaymentMethod()" ng-init="updatePaymentMethod()">
									<option value="0" ng-if="cash=='1'" ng-selected="payment_method=='0'">{{ trans('messages.store.cash_paid_at_delivery') }}</option>
									<option ng-if="stripe=='1'" value="1" ng-selected="payment_method=='1'">{{ trans('messages.store.debit_or_credit_card') }}</option>
									<option value="2" ng-if="paypal=='1'" ng-selected="payment_method=='2'">Paypal</option>
								</select>
								<div ng-show="payment_details" id="payment_detail" ng-init="payment_details={{json_encode($payment_detail)}};">
								...
								<div class="payment-method mt-3" ng-if="stripe=='1' && payment_method==1" data-toggle="modal" data-target="#payment-modal">
									<a href="javascript:void(0)">
										<i class="icon icon-add mr-2"></i>
										{{ trans('messages.store.add_payment_method') }}
									</a>
								</div>
								<div class="remove_promo mt-3" ng-hide="order_data.promo_amount < 1" ng-tets="@{{order_data.promo_id}}">
										<input type="hidden" value="@{{order_data.promo_id}}" id="promo_code_id">	 
										<button ng-click="remove_promo()" type="button" class="btn btn-theme mt-3 mt-md-0 promo_remove">{{ trans('messages.profile.remove_promo') }}</button>
								</div>
								<div class="d-flex align-items-center my-4">
									<span>Wallet</span>
									<input type="checkbox" name="wallet" id="wallet"  ng-model="wallet" ng-checked="apply_wallet()" style="height: 20px;width: 20px;margin: 0 20px;" /> 
								</div>
								...
								<div class="mt-3 driver_tips" ng-int="order_data.tips =0.00">
									<a href="javascript:void(0)" ng-click="show_tips()" class="theme-color tips_btn_show"> 
										<span ng-if="order_data.tips == 0.00">{{ trans('messages.store.add_driver_tips') }}</span>
										<span ng-if="order_data.tips != 0.00">{{ trans('messages.store.edit_driver_tips') }}</span>
									</a>
									<div class="add-tips" style="display: none">
										<form id="apply_driver_tips" >
											@csrf
											<input class="tips_code_val col-md-6 col-12 mr-0 mr-md-3" type="text" name="tips" id='tips' maxlength="6" value=@{{order_data.tips}}  onkeypress="return isNumber(event)">
											<button ng-click="apply_tips()" type="button" class="btn btn-theme mt-3 mt-md-0">{{ trans('messages.profile.apply') }}</button>
											<span class="tips_code_error text-danger d-none">{{ trans('messages.store.please_enter_driver_tips') }}</span>

									 <div class="remove_promo mt-3" ng-hide="order_data.tips <= 0.00" > 
										<button ng-click="remove_tips()" type="button" class="btn btn-theme mt-3 mt-md-0 promo_remove">{{ trans('messages.profile.remove_tips') }}</button>
											</div>
										</form>
									</div>
								...
								<p class="tips_error text-danger d-none mt-2"></p>
								<p class="tips_code_success text-success d-none mt-2"></p>
					...
					<div class="cart-scroll">
						<div class="checkout-item d-flex align-items-start" ng-repeat="order_row in order_data.items">
							...
							<div class="checkout-name col-9 pl-md-0">
								<h4 class="d-md-flex justify-content-between">
									<span class="col-md-7 p-0">
										...
										<div class="modifier-info d-flex" ng-repeat="modifier in order_row.modifier">
											...
											@endif
											<span class="d-inline-block text-nowrap ml-1"> ( {!!$store_details->currency->currency_code!!} @{{modifier.price}} ) </span>
										</div>
									</span>
									<span class="col-md-5 d-inline-block text-md-right p-0">
										<span>{!!$store_details->currency->currency_code!!}</span>
										<span class="d-inline-block">
								...
								<a class="theme-color" data-remove="@{{$index}}" href="" id="remove_order" ng-click="remove_sesion_data($index)">{{ trans('messages.remove') }}</a>
							</div>
						</div>
					</div>
					...
					<div class="total" ng-show="order_data">
						<div class="checkout-total">
							...
							<div id="subtotal">
								<div class="checkout-total d-flex align-items-center" ng-init="total_count_order = {{count(session('order_data') ?? [])-1}}">
									...
									<div class="col-5 text-right">
										<h3 ><span>{!!$store_details->currency->currency_code!!}</span> <span id="total_price_dat">@{{order_data.subtotal | number : 2 }}</span></h3>
									</div>
								</div>
							</div>
							<div class="d-flex align-items-center" >
								...
								<div class="col-5 text-right" >
									<p><span class="currency1">{!!$store_details->currency->currency_code!!}</span> <span id="tax_amoun">@{{order_data.tax | number : 2 }}</span></p>
								</div>
							</div>
							...
							<div class="d-flex align-items-center">
								...
								<div class="col-5 text-right">
									<p><span class="currency1">{!!$store_details->currency->currency_code!!}</span> <span id="booking_fee_amoun">@{{order_data.booking_fee | number : 2 }}</span></p>
								</div>
							</div>
							<div ng-show="order_data.delivery_fee > 0" class="d-flex align-items-center" id='delivery_fee_ty'>
								<div class="col-7">
									<p>{{ trans('messages.profile_orders.delivery_fee') }}</p>
								</div>
								<div class="col-5 text-right">
									<p><span class="currency1">{!!$store_details->currency->currency_code!!}</span> <span id="delivery_fee_amoun">@{{order_data.delivery_fee | number : 2 }}</span></p>
								</div>
							</div>
							<div class="d-flex align-items-center" ng-show="order_data.penalty > 0">
								...
								<div class="col-5 text-right" >
									<p><span class="currency1">{!!$store_details->currency->currency_code!!}</span> @{{order_data.penalty}}</p>
								</div>
							</div>
							<div class="d-flex align-items-center" ng-show="order_data.wallet_amount > 0">
								<div class="col-7">
									<p>{{ trans('messages.profile.wallet_amount')}}</p>
								</div>
								<div class="col-5 text-right" >
									<p><span class="currency1"> - {!!$store_details->currency->currency_code!!}</span> @{{order_data.wallet_amount}}</p>
								</div>
							</div>
							<div ng-show="order_data.tips > 0" class="d-flex align-items-center" >
								<div class="col-7">
									<p>{{ trans('messages.profile_orders.tips') }}</p>
								</div>
								<div class="col-5 text-right" >
									<p><span class="currency1">{!!$store_details->currency->currency_code!!}</span> <span id="tax_amoun">@{{order_data.tips | number : 2 }}</span></p>
								</div>
							</div>
							<div ng-show="order_data.promo_amount > 0" class="d-flex align-items-center" >
								<div class="col-7">
									<p>{{ trans('messages.profile_orders.promo_amount') }}</p>
								</div>
								<div class="col-5 text-right" >
									<p><span class="currency1"> - {!!$store_details->currency->currency_code!!}</span> <span id="tax_amoun"> @{{order_data.promo_amount | number : 2 }}</span></p>
								</div>
							</div>
							...
							<div class="d-flex align-items-center">
								...
								<div class="col-5 text-right">
									<h3><span class="currency1">{!!$store_details->currency->currency_code!!}</span> <span id="total_price_dat">@{{order_data.total_price | number:'2'}}</span></h3>
									<input type="hidden" name="delivery_type_radio" id ="delivery_type_radio" value="@{{order_data.delivery_type}}">
								</div>
							</div>
					...
					<div class="card-form">
						<form>
							<div class="form-group card-number">
								...
								<input type="text" name="card_number" id="card_number" placeholder="{{ trans('messages.profile.card_number') }}"  maxlength="16"  minlength="16"/>
							</div>
							...
							<button type="submit" class="w-100 btn btn-theme" id="payment_card" ng-click='updateCardDetails();'>{{ trans('messages.store.add_card') }}</button>
						</form>
