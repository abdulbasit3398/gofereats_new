			...
			<div class="profile-tab">
				...
				<div class="tab-content" id="myTabContent">
					<div  class="tab-pane fade show active" id="profile" role="tabpanel" aria-labelledby="profile-tab">
						<div class="custom-panel mt-4 mb-5 mx-auto col-md-11 col-lg-8">
									...
									<div class="d-flex w-100 mobile-error error_clone">
										...
										<select id="phone_code" name="phone_code" class="form-control">
						                    @foreach ($country as $key => $country)
						                        <option value="{{ $country->phone_code }}" data-id="{{ $country->id }}"  {{ $country->phone_code == $basic->country_code ? 'selected' : '' }} >{{ $country->name }}</option>
						                    @endforeach
						                </select>
							...
							<div class="form-group d-md-flex">
								...
								<div class="col-md-8">
								<input type="text" autocomplete="off" id="dob_datepicker" name='dob' value="{{$basic->date_of_birth?date(site_setting('site_date_format'),strtotime($basic->date_of_birth)):''}}">
							...
							<div class="form-group d-md-flex">
								<label class="col-md-4 col-form-label">
									@lang('admin_messages.delivery_type')
									<span class="required">*</span>
								</label>

								<div class="col-md-8">
									<div class="form-group d-md-flex">
										@foreach($delivery_typ as $delivery_key => $delivery_value)
										<div class="form-check mr-3 p-0">
											<label class="form-check-label">
												{!! Form::checkbox('delivery_type[]',$delivery_key,in_array($delivery_key,explode(',',$store->delivery_type)), ['class'=>'form-check-input','data-error-placement'=>"container" ,'data-error-container'=>".delivery_error"]); !!}
									...
									<span class="delivery_error"> </span>
							</div>
							</div>
							<div class="form-group d-md-flex">
								<div class="col-md-4">
							...
							<div class="form-group d-md-flex">
								...
								<div class="col-md-8">
									{!! Form::select('currency_code',$currency_select,@$store->currency_code, ['placeholder' => trans('admin_messages.select')]); !!}
								...
								<div class="col-12 p-0 cuisine_err">
									<div class="form-group mt-3 cuisine row ">
										...
										<div class="form-check col-md-6">
											<label class="form-check-label">
												{!! Form::checkbox('cuisine[]',$cuisine_key,in_array($cuisine_key,$store_cuisine), ['class'=>'form-check-input','data-error-placement'=>"container" ,'data-error-container'=>".cuisine_error"]); !!}
												<span class="form-check-sign">
													<span class="check"></span>
												</span>
												{{$cuisine_value}}
							...
							<div class="form-group d-md-flex" ng-init="latitude='{{ isset($address->latitude)?$address->latitude:''}}';longitude='{{ isset($address->longitude)?$address->longitude:''}}';verify_code=''">
								...
								<div class="col-md-8">
									...
									<span class="upload_text@{{$index}}" id="file_text_1"></span>
									</div>
					...
					<div class="tab-pane fade" id="document" role="tabpanel" aria-labelledby="document-tab">
						<div class="custom-panel mt-4 mb-5 mx-auto col-md-11 col-lg-8">
							{!! Form::open(['url'=>route('restaurant.update_documents'),'method'=>'post','class'=>'mt-2' , 'id'=>'store_documents','files' => true])!!}
							@csrf
			<!--
			<div class="form-group d-md-flex">
						<div class="col-md-5">
				...
				<div ng-repeat="document in all_document" ng-cloak>
					...
					<div class="d-md-flex mt-3">
						...
						<div class="col-md-7">
							...
							<a class="choose_file_type document_choose@{{$index}}" id="@{{$index}}" onclick="documentImage(id)"><span id="doc_name@{{$index}}">{{trans('messages.profile.choose_file')}}</span></a>
							<span class="upload_text" id="file_text@{{$index}}"></span>
                                  <div style="">
              		...
              		<div class="mb-4 mb-md-3 d-md-flex align-items-start select-day menu-view" ng-repeat="open_time in open_time_timing">
              			<div class="select">
              				<select name="day[]" ng-model="open_time.day" id="select_day_@{{$index}}">
              					...
              					<option value="@{{key}}" ng-selected="open_time.day==key" ng-repeat="(key,value) in day_name track by $index" ng-if="( key | checkKeyValueUsedInStore : 'day': open_time_timing) || open_time.day==key">@{{value}}</option>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#dob_datepicker").datepicker({
			dateFormat: '{!! site_setting("ui_date_format") !!}',
			maxDate: '0',
		});
	});
    ...
    function documentImage(input) {
    	...
    	$('#document_file'+input).change(function(evt) {
    		var fileName = $(this).val().split('\\')[$(this).val().split('\\').length - 1];
        		$('.document_choose'+input).css("color","#fff");
        		$('.document_choose'+input).css("background-color","#43A422");
        		

        		$('#doc_name'+input).text(Lang.get('js_messages.file.file_attached'));
        		$('#file_text'+input).text(fileName);
        		$('span.upload_text'+input).attr('title',fileName)
    		});
