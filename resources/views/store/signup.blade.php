								...
								<div class="d-flex w-100">
									<div class="select mob-select col-md-3">
										...
										<select id="phone_code" name="country" class="form-control">
											...
											<option value="{{ $country->phone_code }}"  data-id="{{ $country->id }}"   {{ $country->phone_code == @session::get('code') ? 'selected' : '' }} >{{ $country->name }}</option>
											@endforeach
										</select>
										<input type="hidden" name="apply_country_code" id="apply_country_code" value=""/>
									</div>
									{!! Form::text('mobile_number','',['placeholder' => trans('messages.store.phone_number'),'class' =>'col-md-9','data-error-placement'=>'container','data-error-container'=>'.mobile-number-error'])!!}
								</div>
			...
			<div class="container">
				<div class="row">
					...
					<div class="col-md-4">
						...
						<h2>{{trans('messages.store.quick_deliver')}}</h2>
						<p>{{trans('messages.store.food_your_customers', ['site_name'=>site_setting('site_name')] ) }}</p>
					</div>
					<div class="col-md-4">
						<div class="res-img">
...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		if($('#phone_code').val())
			$('#apply_country_code').val(phone_code );
		console.log($('#apply_country_code').val());
	});		
