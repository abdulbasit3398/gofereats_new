			...
			<div class="verify-steps">
				<ul>
					<li class="{{$document? 'completed':''}}">
						<a href="{{route('restaurant.profile','#document')}}">
					...
					<li class="{{$open_time? 'completed':''}}">
						<a href="{{route('restaurant.profile','#open_time')}}">
							<i class="icon icon-success"></i>
							<i class="icon icon-cancel-button"></i>
		...
		<div id="sales">
			...
			<div class="panel-content my-3 my-md-5">
				<div class="tab-pane fade active" id="weekly" role="tabpanel" aria-labelledby="weekly-tab">
					<div class="d-md-flex align-items-center justify-content-between">
						<div class="net-pay col-md-4">
							<h2> {{ $currency_code }} {{$last_seven_total_payouts}}</h2>
							<p>{{trans('messages.store_dashboard.net_payout')}}</p>
							<h2> {{ $currency_code }} {{$store_owe_amount}}</h2>
							<p>{{trans('messages.store_dashboard.owe_amount')}}</p>
						</div>

						<div class="net-chart col-md-8 mt-5 mt-md-0">
							@if(isset($seven_chart))
								<center>
									{!! $seven_chart->container() !!}
								</center>
							@else
				...
				<div class="tab-pane fade active " id="monthly" role="tabpanel" aria-labelledby="monthly-tab">
					<div class="d-md-flex align-items-center justify-content-between">
						<div class="net-pay col-md-4">
							<h2>{{ $currency_code }}{{$last_thirty_total_payouts}}</h2>
							<p>{{trans('messages.store_dashboard.net_payout')}}</p>
						</div>
						<div class="net-chart col-md-8 mt-5 mt-md-0">
							@if(isset($thirty_chart))
								<center>
									{!! $thirty_chart->container() !!}
								</center>
							@else
							...
							<div class="d-block row d-lg-flex align-items-center">
								...
								<div class="col-12 col-lg-6 mt-4 mt-lg-0">
									<div class="hrs-info">
										...
										<p class="light-color">{{trans('messages.store_dashboard.customers_like_your_food')}}</p>
									</div>
								</div>
							</div>
						</div>
								...
								<tr>
									...
									<td>
										<div class="d-flex align-items-center">
											<div class="bar">
												<?php $prasantage = ($reviews['prasantage'] == 0) ? 100 :$reviews['prasantage'];  ?>
												<span style="width: {{ $prasantage }}%" class="bar-process @if($reviews['prasantage']==100)  green @elseif($reviews['prasantage']>=50) yellow @elseif($reviews['prasantage']<50) red @endif"></span>
											</div>
									...
									<td>
										<div class="feedbacks">
											...
											@endforeach
											@endif
							@if(count($reviews['negative_comments']) > 0)
								
								@foreach($reviews['negative_comments'] as $comments)
								<p>{{$comments}} </p>
								@endforeach
								({{count($reviews['negative_comments'])}})
							@endif
								</div>
									</td>
									<td class="text-right" >
										<input type="hidden" name="comments" value="{{$reviews['review_comments']}}" id="comments_{{$review_id}}">
										@if(($reviews['review_comments'] != '' ))
										<a href="javascript:void(0)"><i ng-click="show_comments({{$review_id}})"  class="icon icon-comment-black-rectangular-speech-bubble-interface-symbol"></i></a>@endif
									</td>
								</tr>
								@endforeach
