...
<main id="site-content" role="main" class="log-user">
	<div class="container">
		...
		<div class="login-form py-5 mb-5 col-md-8 col-lg-6 mx-auto">
			...
			{!! Form::open(['url'=>route('restaurant.password'),'method'=>'post','class'=>'mt-4' , 'id'=>'login_form'])!!}
			@csrf
			<div class="form-group" ng-init="textInputPassword='{{$password}}'">
			...
			<a href="{{route('restaurant.forget_password')}}">{{trans('messages.profile.forget_password')}}<span class="qust">{{ trans('messages.store.ques_mark') }}</span> </a> <small> <a href="{{route('help_page',current_page())}}">{{trans('messages.profile.get_help')}}</a></small>
