...
<div class="content">
    ...
    <div style="float: right;text-align: right;" class="col-md-12">
      @checkPermission('create-restaurant')
      <a class="btn btn-success" href="{{route('admin.add_restaurant')}}"> {{ trans('admin_messages.add_restaurant') }} </a>
      @endcheckPermission
    </div>
    <div class="card-body ">
...
<script>
  var oTable = $('#statement_table').DataTable({
    ...
    ajax: {
      url: ajax_url_list['all_restaurants'],
      data: function (d) {
    ...
    columns: [
    {data: 'id', name: 'store.id', title: 'ID'},
    {data: 'name',name: 'user.name',title: 'Name'},
    {data: 'store_name',name: 'store.name',title: 'Restaurant Name'},
    {data: 'email',name: 'user.email ',title: 'Email'},
    {data: 'user_status',name: 'user_status',title: 'User Status'},
    {data: 'delivery_type',name: 'delivery_type',title: 'Delivery Type'},
    {data: 'store_status',name: 'store_status',title: 'Restaurant Status'},
