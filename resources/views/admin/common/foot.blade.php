...
<!--  Google Maps Plugin  -->
<script type="text/javascript" src="//maps.googleapis.com/maps/api/js?key={{site_setting('google_api_key')}}&libraries=places"></script>
...
<script src="{{asset('js/moment-range.js?v='.$version)}}" type="text/javascript"></script>
...
<script src="{{ url('admin_assets/js/language.js') }}"></script>
<script src="{{ url('admin_assets/js/datatable_edit_able.js') }}"></script>
...
<script type="text/javascript">
  var ajax_url_list = {
    ...
    all_restaurants            : '{{route("admin.all_restaurants")}}',
    ...
    menu_locale                : '{{route("admin.menu_locale")}}',
    ...
    category_type              : '{{route("admin.category_type")}}',
    language_update            : "{{route('admin.language_update')}}",
    delete_documents           : "{{route('admin.delete_documents')}}",
    get_category               : "{{route('admin.get_category')}}",
    get_item                   : "{{route('admin.get_item')}}",
    get_user_list              : "{{route('admin.get_user_list')}}",
  };
...
@if(!in_array(Route::CurrentRouteName(),['admin.send_message']))
<script>
