...
class OrderDataTable extends DataTable {
		...
		return datatables($query)
			->addColumn('action', function ($query) {
				if (get_current_login_user() == 'admin') {
					...
					return '<a title="' . trans('admin_messages.edit') . '"  href="' . route('restaurant.edit_order', $query->id) . '" ><i class="material-icons">edit</i></a>';
				}
			})
	...
	public function query() {
		if (get_current_login_user() == 'admin') {
			return Order::get();
		} else if (get_current_login_user() == 'restaurant') {
