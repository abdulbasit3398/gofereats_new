...
class PayoutDayReportDataTable extends DataTable {
	...
	public function query() {
		...
		if ($user->type_text == 'store') {
			$order = Order::with('payout_table')->where('store_id', $user_id)->whereNotIn('status',[2,4]);
		} else {
			$order = Order::with('payout_table')->where('driver_id', $user_id)->whereNotIn('status',[2,4]);
		}
		return $order->history()
			->whereHas('payout_table', function ($query) use($from ,$to) {
