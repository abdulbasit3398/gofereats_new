...
class SliderController extends Controller {
	...
	public function home_slider(Request $request) 
	{
		...
		else 
		{
			...
			{
				if($request->id) {
					$active_records = HomeSlider::where('id','!=',$request->id)->where('status',1)->count();
					if($active_records < 1 &&  $request->status == 0){
						return back()->withErrors(['status'=>'At least one slider is active status'])->withInput(); 						
					}
	...
	{
		$total = HomeSlider::where('id','!=',$request->id)->where('status','1')->count();
		if($total < 1){
