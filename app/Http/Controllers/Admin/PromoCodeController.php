...
class PromoCodeController extends Controller {
	...
	public function edit(Request $request) {
		if ($request->getMethod() == 'GET') {
			...
			$this->view_data['form_action'] = route('admin.edit_promo', $request->id);
			$this->view_data['promo'] = PromoCode::findOrFail($request->id);
			$this->view_data['cur_date'] = date('Y-m-d');
			...
			if($all_variables['convert_end_date'] >= date('Y-m-d') && $all_variables['convert_start_date'] < date('Y-m-d') )
				$rules['convert_start_date'] = 'required';
			if ($request->promo_type == 0) {
				$rules['price'] = 'required|numeric|min:0|not_in:0';
			} else if ($request->promo_type == 1) {
				$rules['percentage'] = 'required|numeric|min:0|max:100';
