...
class StripePayment
{
	...
	protected $return_data = array('status' => 'success', 'status_message' => '');
	/**
    ...
    {
    	$this->return_data = array('status' => 'success', 'status_message' => '');
    }
        ...
        try {
            ...
            try {
                ...
                if(!$newCustomer->success) {
                    \View::share('paypal_error', $newCustomer->message ?  $newCustomer->message:'Authentication failed');
                    flash_message('danger', $newCustomer->message);
                    return back();
                }
                ...
                if($e instanceOf \Braintree\Exception\Authentication) {
                    \View::share('paypal_error', $e->getMessage() ?$e->getMessage():'Authentication failed');
                    return back();
                }
                \View::share('paypal_error', $e->getMessage() ? $e->getMessage(): 'Authentication failed');
                return back();
     ...
     * @return Boolean
     */
    public function PaypalPayment($amount,$nonce)
    {
        try {
            $gateway = resolve('braintree_paypal');
     ...
     * Paypal Refund
     *
     * @param Amount - User order amount
     * @param String Transaction Id
     * @return Boolean
     */

    public function RefundPaypal($transaction_id,$amount = null)
    {
        try {
            $gateway = resolve('braintree_paypal');
            if($amount == null){
