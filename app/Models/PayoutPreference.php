...
/**
 * PayoutPreference Model
 *
 * @package     GoferEats
 * @subpackage  Model
 * @category    PayoutPreference
...
class PayoutPreference extends Model
{
	...
	public function getDocumentAttribute()
	{
		...
		$file_path = \Storage::url('images/payout_documents/'.$user_id.'/'.$image_name);
		return asset($file_path);
	}

	public function getAdditionalDocumentAttribute()
	{
		$image_name = $this->attributes['additional_document_image'];
		$user_id = $this->attributes['user_id'];
		$file_path = \Storage::url('images/payout_documents/'.$user_id.'/'.$image_name);
		return asset($file_path);
	}
	...
	public static function getMandatory($country = 'US')
	{
		...
		$mandatory['AU'] = array('account_number' => 'required', 'account_holder_name' => 'required', 'currency' => 'required');
		...
		$mandatory['IN'] = array('ifsc_code' => 'required', 'account_number' => 'required', 'account_holder_name' => 'required', 'currency' => 'required' ,'personal_id' => 'required');
	...
	{
		$mandatory['AU'] = array('account_number' => 'required', 'account_holder_name' => 'required', 'currency' => 'required', 'routing_number' => 'required');
		...
		$mandatory['OT'] = array('account_number' => 'required', 'bank_name' => 'required', 'account_holder_name' => 'required','branch_name' => 'required');
		$mandatory['IN'] = array('ifsc_code' => 'required', 'account_number' => 'required', 'account_holder_name' => 'required', 'currency' => 'required' ,'personal_id' => 'required');
		// $mandatory['IN'] = array('iban' => 'required', 'account_holder_name' => 'required', 'currency' => 'required','account_number'=>'required');
	...
	public static function getAllMandatory()
	{
		...
		$mandatory['AU'] = array('Account Number');
		...
		$mandatory['IN'] =  array('IFSC Code', 'Account Number');
		$mandatory['OT'] = array('Account Holder Name', 'Account Number','','Bank Name','Branch Name');
		return $mandatory;
	}

	public function getAddressKanjiAttribute()
	{
		$kanji_data = json_decode($this->attributes['address_kanji'],true);
		return $kanji_data;
	}
}
