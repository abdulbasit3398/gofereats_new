...
trait CurrencyConversion
{
 ...
 public function getSessionOrDefaultCode() {
    ...
    }else if(request()->segment(1) == 'restaurant') {
      $currency_code = auth()->guard('restaurant')->user()->currency_code->code;
    }else if(request()->segment(1) == 'driver') {
      if(session()->get('driver_currency'))
      {
        $currency_code =  session()->get('driver_currency');
      }
      else
      {
         $currency_code = Currency::defaultCurrency()->first()->code;
  ...
  public function currency_convert($from = '', $to = '', $price = 0)
  {
    ...
    $rate = Currency::whereCode(trim($from))->first()->rate;
    $session_rate = Currency::whereCode(trim($to))->first()->rate;
