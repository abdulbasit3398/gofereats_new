...
class Store extends Model {
	...
	protected function updateStoreTimeZone()
	{
	...
	public function store_cuisine()
	{
		return $this->hasMany('App\Models\StoreCuisine', 'store_id', 'id');
	}
	...
	{
		$this->updateStoreTimeZone();
		return $this->belongsTo('App\Models\StoreTime', 'id', 'store_id')->isActive()->where('day', date('N'));
	}
	...
	public function getBannerAttribute()
	{
		...
		if ($image) {
			...
			$image = [
				...
				'large' => $url . '/' . $filename . '_' . $this->image_size['4'] . '.' . $extension,
			];
	...
	public function preparationTime()
	{
		...
		$day = date('N', $date);
		$store_preparation_time = $this->store_preparation_time()->isActive()->where('day', $day)->first();
	...
	public function getStorePreparationTime($date)
	{
		$date = strtotime($date);
		$day = date('N', $date);
		$store_preparation_time = StorePreparationTime::where('day', $day)->where('store_id', $this->attributes['id'])->where('status',1)->first();
		if ($store_preparation_time) {
			$preparation_time = $store_preparation_time->attributes['max_time'];
		}
		else {
	...
	{
		$mobile_verify = (site_setting('otp_verification') == 'Yes') ? $this->user->mobile_no_verify : '1';
		if ($mobile_verify == 1 && $this->user->date_of_birth && $this->attributes['description']  && $this->attributes['price_rating'] ) {
			return true;
		} else {
			return false;
		}
	...
	public function service_type1() {
		return $this->belongsTo('App\Models\ServiceType', 'service_type', 'id');
	}

	public function getDeliveryTypeArrayAttribute()
	{
		$delivery_type = explode(",",$this->delivery_type);
		$count = count($delivery_type);
		if($count > 1 )
		{
			return "both";
		}
		else 
		{
			return $delivery_type[0];
				...
				// $delivey_vvalue = array_intersect($delivery_type, $this->deliveryArray);
		// $deliveryType=[];
		// foreach ($delivey_vvalue as $key => $value) {
		// 	$deliveryType[] = ['key' => $key, 'value' => $value] ;
		// }
