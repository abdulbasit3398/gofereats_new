...
class UserAddress extends Model
{
	...
	protected $appends = ['address1', 'default_timezone'];
	...
	public function getStaticMapAttribute() {
		...
		if(isset($this->attributes['latitude'])) {
			return 'https://maps.googleapis.com/maps/api/staticmap?center=' . $this->attributes['latitude'] . ',' . $this->attributes['longitude'] . '&markers=icon:' . url('images/map_green.svg') . '|color:red|label:C|' . $this->attributes['latitude'] . ',' . $this->attributes['longitude'] . '&zoom=12&size=100x100&key=' . site_setting('google_api_key');
		} else {
			return '';
		}
	}
