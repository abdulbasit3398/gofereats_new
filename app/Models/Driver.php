...
class Driver extends Model
{
	...
	public function documents() {
		return $this->hasMany('App\Models\File', 'source_id', 'user_id')->whereIn('type', array_values($this->document_type_array));
	}
	...
	public function getDriverContactAttribute() {
		...
		$phone_code = Country::where('code',$user->country_code)->first();
		if ($user) {
			if(isset($phone_code->phone_code))
				return $phone_code->phone_code." ". $user->mobile_number;
			else
				return "1 " .$user->mobile_number;
		}
		return '';
	}
	//vehicle_image
	...
	public function getDriverProfilePictureAttribute() {
		...
		$file_image = File::where(['source_id' => $this->attributes['user_id'], 'type' => $filetype->id])->first();
